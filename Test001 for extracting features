...
def analyze_audio(duration=5, sample_rate=44100):
    ...
    try:
        ...
        # Amplitude (RMS)
        amplitude = librosa.feature.rms(y=audio_data).mean()
        amplitude_db = librosa.amplitude_to_db([amplitude], ref=np.max)[0]  # Convert to dB
        ...
        return key, amplitude_db, octave, tempo
...
if __name__ == "__main__":
    ...
    if analysis_result:
        key, amplitude_db, octave, tempo = analysis_result
        print(f"Key Estimate: {key}")
        print(f"Amplitude (dB): {amplitude_db:.2f}")
        print(f"Octave: {octave}")
        print(f"Tempo (BPM): {float(tempo[0]):.2f}")
