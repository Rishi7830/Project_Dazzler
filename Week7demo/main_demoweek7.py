...
# ====================================================================
# ADJUSTED CONSTANTS FOR FASTER RESPONSE
# ====================================================================
SR = 44100
WINDOW_SEC = 2.5  # Reduced Window: Still long enough for decent feature extraction
HOP_SEC = 0.25    # CRITICAL: Reduced Hop Size for faster light updates (4 times per second)
...
def process_audio_chunk(chunk, buffer, genre):
    ...
    # Feature Extraction (These take the full WINDOW_SIZE audio)
        ...
        while not stop_event.is_set():
            try:
                if not mood_queue.empty():
                    # Get the latest mood data
                    mood, color, loudness, energy = mood_queue.get_nowait()
                    
                    # Log the DMX change for debugging
                    print(f"--> Light Change: Mood={mood}, Loudness={loudness:.2f}dB, Energy={energy}")
                    
                    # Use DMX functions based on your frequency/loudness logic
                    # CRITICAL CHANGE: The 'duration' parameter is REMOVED to prevent long blocking/flickering
                    # The update rate is now controlled by HOP_SEC in the main thread.
                    if loudness >= LOUDNESS_HIGH_THRESHOLD:
                        # High energy/loudness uses high frequency/dynamic DMX
                        run_high_frequency_dmx_chunk(dmx, color, energy)
                    elif loudness <= LOUDNESS_LOW_THRESHOLD:
                        # Low energy/loudness uses low frequency/smooth DMX
                        run_low_frequency_dmx_chunk(dmx, color, energy)
                    else:
                        # Mid energy/loudness uses low frequency/smooth DMX
                        run_low_frequency_dmx_chunk(dmx, color, energy)

            except queue.Empty:
                # No new mood data, continue holding the current state
                pass
...
def process_single_file(filepath, genre, dmx_port):
    ...
    chunk_moods = []
    mood_queue = queue.Queue(maxsize=20) # Increased queue size for faster hop rate
    ...
    try:
        ...
        while pos < len(y):
            ...
            try:
                ...
                # Non-blocking put to queue - crucial for real-time
            ...
            # ===============================================================
            # TIMING CONTROL - Ensures the next chunk starts on time
            # ===============================================================
