...
from Loudness_detection_Week7_1311 import detect_loudness
...
from mood_color_map_1311 import map_mood_to_genre_color, get_energy_level, generate_color_palette
from High_Frequency_DMX_1311 import run_high_frequency_dmx_chunk
from Low_Frequency_DMX_1311 import run_low_frequency_dmx_chunk
...
LOUDNESS_HIGH_THRESHOLD = -15  # Adjusted for better sensitivity
LOUDNESS_LOW_THRESHOLD = -35   # Adjusted for better sensitivity

def get_user_inputs():
    """Gets genre, audio file path, and DMX port from the user."""
...
def save_moods_to_csv(mood_list, filename):
    """Saves the mood analysis results to a CSV file."""
...
def process_audio_chunk(chunk, buffer, genre):
    """Processes a chunk of audio to extract features and predict mood."""
    buffer.update(chunk)
    windowed_audio = buffer.get_window()

    # Feature Extraction
    mode, key = detect_mode_key(windowed_audio)
    tempo = detect_tempo(windowed_audio)
    loudness = detect_loudness(windowed_audio)
    bpm, rhythm_index = extract_rhythm(windowed_audio)
    hpcp_mean, _, _, _, harmony_class = extract_harmony(windowed_audio)

    # Mood Prediction
    features_processed = preprocess_features(mode, tempo, loudness, rhythm_index, harmony_class)
    mood = predict_mood(features_processed)
    mood_color = map_mood_to_genre_color(mood, genre)
    energy_level = get_energy_level(loudness)
    color_palette = generate_color_palette(mood_color)

    return mood, mood_color, loudness, energy_level, color_palette

def lighting_controller_thread(dmx_port, mood_queue, stop_event):
    """A thread that controls the DMX lights based on mood data."""
    try:
        dmx = SimpleDMX(port=dmx_port)
        if not dmx.ser:
            print("DMX controller not initialized. Lighting thread will now exit.")
            return
        ...
        while not stop_event.is_set():
            try:
                if not mood_queue.empty():
                    mood, color, loudness, energy, palette = mood_queue.get_nowait()
                    ...
                    # Decide whether to strobe based on loudness
                    is_strobe_active = (loudness >= LOUDNESS_HIGH_THRESHOLD)

                    dmx.update_lighting(rgbw_color, is_strobe=is_strobe_active)
...
def process_single_file(filepath, genre, dmx_port):
    """Loads an audio file and processes it chunk by chunk for real-time analysis."""
    ...
    try:
        ...
        for chunk in librosa.util.frame(y, frame_length=HOP_SIZE, hop_length=HOP_SIZE).T:
            start_time = time.time()
            if len(chunk) < HOP_SIZE:
                chunk = np.pad(chunk, (0, HOP_SIZE - len(chunk)), 'constant')

            try:
                mood, mood_color, loudness, energy, palette = process_audio_chunk(chunk, buffer, genre)
                timestamp = pos / sr
                chunk_moods.append((timestamp, mood, mood_color, loudness, energy))
                if not mood_queue.full():
                    mood_queue.put((mood, mood_color, loudness, energy, palette))
