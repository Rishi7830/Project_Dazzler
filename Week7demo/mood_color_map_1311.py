...
def generate_color_palette(base_color, num_colors=5):
    """
    Generates a palette of colors around a base color.
    """
    palette = [base_color]
    r, g, b = base_color
    for _ in range(num_colors - 1):
        # Create variations by slightly shifting the hue and saturation
        r_new = min(255, max(0, r + random.randint(-40, 40)))
        g_new = min(255, max(0, g + random.randint(-40, 40)))
        b_new = min(255, max(0, b + random.randint(-40, 40)))
        palette.append((r_new, g_new, b_new))
    return palette
