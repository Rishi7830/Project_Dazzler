...
FINAL VERSION: Includes Real-Time Synchronization and Loudness-Based Color Mapping with dynamic hue cycling.
"""
...
def stream_mp3_realtime(
    ...
    """
    Stream MP3 in real time, analyze features per window,
    update DMX lighting with smooth color and dynamic hue speed.
    """
    ...
    try:
        while True:
            ...
            while analysis_buffer.size >= chunk_samples:
                ...
                # Logging
                print(f"[{time_position:6.2f}s] L:{loudness:5.2f}dB | T:{tempo:3.0f}bpm | RGB{rgbw[:3]} | Speed:{hue_speed:.2f}")

                results.append({
                    "time_position": time_position,
                    "features": {"mode": mode, "key": key, "tempo": float(tempo), "loudness": float(loudness)},
                    "lighting": {"mapped_rgbw": rgbw, "hue_speed": float(hue_speed)}
                })
