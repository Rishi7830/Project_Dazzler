...
Fixes: Corrected timing to prevent analysis from running faster than the music.
"""
...
# Import only necessary functions from color_mapper (Mood logic removed)
...
def map_features_to_genre_color(feature_output, genre):
    ...
    try:
        # Use a simple cycle based on a hash of the feature output to pick a color
        if isinstance(feature_output, tuple) or isinstance(feature_output, list):
             # Use the first element's float value for a cycling index
...
def stream_mp3_realtime(
    ...
    try:
        while True:
            # 1. Read a block from ffmpeg (This acts as the main clock signal)
            ...
            while analysis_buffer.size >= chunk_samples:
                ...
                # If sleep_needed is negative, the analysis is behind schedule, and we continue immediately.
                # -------------------------------------
                ...
                # --- Logging & Data Recording (Using str() to prevent format errors) ---
